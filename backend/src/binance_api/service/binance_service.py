from src.binance_api.binance_http import BinanceAPI, BinanceHTTPMethods
from src.binance_api.schemas.binance_schemas import TimeFramePercentChanges
from src.binance_api.binance_utils import BinanceTransformer
from src.binance_api.binance_websockets import BinanceWebSocketMethods, WSConnectionManager
from src.core.settings import settings
from fastapi import Depends, WebSocket, WebSocketDisconnect
from src.investment.helpers import InvestmentUtils
from src.portfolio.domain.repos.repo import PortfolioRepo
...
class BinanceService:
    ...
    async def get_all_symbols(self) -> list[str]:
        ...
        Return list tickers
        """
        response = await self.binance_request.get(settings.app.binance_ticker_price_url)
        if response:
    ...
    async def ticker_price_changes(
        ...
        timeframes = ['1d', '7d']
        tm_ticker_prices = {}
        print(await self.portfolio_repo.get_unique_user_ticker(user_id=user_id))
